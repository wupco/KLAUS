...
func (ma *mutationArgs) chooseArgbyIdx(Idx int) (Arg, ArgCtx) {
	arg := ma.args[Idx]
	return arg.arg, arg.ctx
}

func Randomsplice(mi *Mutateinfo) { //interface
	for i := len(mi.Targetlist) - 1; i > 0; i-- {
		//s = append(s, mi.Targetlist[i])
		num := rand.Intn(i + 1)
		mi.Targetlist[i], mi.Targetlist[num] = mi.Targetlist[num], mi.Targetlist[i]
	}
	for i := len(mi.Waitlist) - 1; i > 0; i-- {
		num := rand.Intn(i + 1)
		//s = append(s, mi.Targetlist[i])
		mi.Waitlist[i], mi.Waitlist[num] = mi.Waitlist[num], mi.Waitlist[i]
	}

}

func (info *Mutateinfo) delete() {
	var r []map[int]int
	for _, elm := range info.Targetlist {
		callIdx := -1
		argIdx := -1
		for key, _ := range elm {
			callIdx = key
...
func (ctx *mutator) resizeMutateinfo(info *Mutateinfo) {
	log.Logf(0, "before resize: targetlist: %d, waitlist: %d\n", info.Targetlist, info.Waitlist)
	info.delete()
	log.Logf(0, "resize result: targetlist: %d, waitlist: %d\n", info.Targetlist, info.Waitlist)
}

func (ctx *mutator) initMutateinfo(info *Mutateinfo) {
	cnt := 0
	ma := &mutationArgs{target: ctx.p.Target}
	...
	info.Targetlist = make([]map[int]int, cnt)
	info.Waitlist = make([]map[int]int, cnt)
	tmplist := make([]map[int]int, 0)
	//panic(fmt.Sprintf("calls: %d", ctx.p.Calls))
	for i, call := range ctx.p.Calls {
		//c := p.Calls[callIdx]
		//updateSizes := true
		ma = &mutationArgs{target: ctx.p.Target}
		ForeachArg(call, ma.collectArg)
		for j, arg := range ma.args {
			//cnt_ += 1
			call = call
			arg = arg
			tmplist = append(tmplist, map[int]int{i: j})
			/*
				if cnt_ > (cnt/2){
				   info.Waitlist = append(info.Waitlist, map[int]int{i: j})
				}else{
				   info.Targetlist = append(info.Targetlist, map[int]int{i: j})
				}*/
		}
	}
	for i := len(tmplist) - 1; i > 0; i-- {
		num := rand.Intn(i + 1)
		//s = append(s, mi.Targetlist[i])
		tmplist[i], tmplist[num] = tmplist[num], tmplist[i]
	}
	//info.Waitlist = tmplist[0:cnt/2]
	info.Waitlist = tmplist[0 : cnt-1]
	info.LastWaitlist = info.Waitlist
	//info.Targetlist = tmplist[cnt/2:cnt]
	info.Targetlist = tmplist[cnt-1 : cnt]
	info.LastTargetlist = info.Targetlist
	//panic(fmt.Sprintf("target list len %d", len(info.Targetlist)))
}

func (ctx *mutator) updateMutateinfo(info *Mutateinfo, ctrlnum int) bool {
	//cnt := 0
	if ctrlnum == 0 {
		info.LastWaitlist = info.Waitlist
		info.LastTargetlist = info.Targetlist
		info.Targetlist = info.Targetlist[:0]
		info.Targetlist = append(info.Targetlist, info.Waitlist[0])
		if len(info.Waitlist) == 1 {
			info.Waitlist = info.Waitlist[:0]
		} else {
			info.Waitlist = info.Waitlist[1:len(info.Waitlist)]
		}
		return true
		/*
			info.LastWaitlist = info.Waitlist
			...
			var r []map[int]int
			info.Targetlist = info.Targetlist[:0]
			wl := len(info.Waitlist)/2
			if len(info.Waitlist) == 1 {
				info.Targetlist = append(info.Targetlist, info.Waitlist[0])
				info.Waitlist = info.Waitlist[:0]
				return true
			}
			for i := 0; i < len(info.Waitlist);i++ {
				if cnt > wl {
					r = append(r, info.Waitlist[i])
					continue
				}
				cnt += 1
				info.Targetlist = append(info.Targetlist, info.Waitlist[i])
				//info.Waitlist = append(info.Waitlist[:i], info.Waitlist[i+1:]...)
			}
			info.Waitlist = r
			return true
		*/
	...
	if ctrlnum == 1 {
		...
		/*
			var r []map[int]int
			info.Waitlist = info.Waitlist[:0]
			tl := len(info.Targetlist)/2
			if len(info.Targetlist) == 1 {
				return false
			}
			for i := 0; i < len(info.Targetlist); i++ {
				if cnt > tl {
					r = append(r, info.Targetlist[i])
					continue
				}
				cnt += 1
				info.Waitlist = append(info.Waitlist, info.Targetlist[i])
				//info.Targetlist = append(info.Targetlist[:i], info.Targetlist[i+1:]...)
			}
			info.Targetlist = r
			return true
		*/
...
func (ctx *mutator) mutateArgbyinfo(info Mutateinfo) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) == 0 {
		return false
	}
	if len(info.Targetlist) == 0 {
		panic("init target error!")
		return false
	}
	for _, target := range info.Targetlist {
		...
		if callIdx == -1 || argIdx == -1 {
			panic(fmt.Sprintf("callIDX:%d, argIDX:%d\n", callIdx, argIdx))
			return false
		}
		c := p.Calls[callIdx]
		updateSizes := false
		...
		if argIdx >= len(ma.args) {
			continue
		}
		arg, argCtx := ma.chooseArgbyIdx(argIdx)
		typ := arg.Type()
		_, isArrayTyp := typ.(*ArrayType)
		_, isBufferTyp := typ.(*BufferType)
		_, isConstTyp := typ.(*ConstType)
		_, isResTyp := typ.(*ResourceType)
		//_, isPtrTyp := typ.(*PtrType)
		//_, isUnionTyp := typ.(*UnionType)
		//_, isStructTyp := typ.(*StructType)
		//panic(fmt.Sprintf("arg nums: %d", len(ma.args)))
		//
		//if callIdx == 1 && argIdx == 1  {
		//	panic(fmt.Sprintf("\narg: %v\n",arg.(*PointerArg).Res))
		//}

		//if callIdx == 1 && argIdx == 1 && isStructTyp {
		//	panic(fmt.Sprintf("\nargtype: struct\n"))
		//}
		//if callIdx == 1 && argIdx == 1 && isUnionTyp {
		//	panic(fmt.Sprintf("\nargtype: Union\n"))
		//}

		//if callIdx == 1 && argIdx == 2 && isPtrTyp {
		//	_, isResTyp := arg.Type().(*ResourceType)
		//	_, isBufTyp := arg.Type().(*BufferType)
		//	if isResTyp{
		//		panic(fmt.Sprintf("\nargtype: isResTyp\n"))
		//	}
		//	if isBufTyp{
		...
		//}//*/
		prio, _ := typ.getMutationPrio(ma.target, arg, true)
		if isResTyp || prio == dontMutate || isConstTyp || !isBufferTyp && !isArrayTyp && arg.Dir() == DirOut || !typ.Varlen() && typ.Size() == 0 {
			continue
		}
...
// func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus []*Prog) {
func (p *Prog) testMutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus []*Prog) {

}
func (p *Prog) NewMutate(rs rand.Source, ncalls int, ct *ChoiceTable, procinfo *Mutateinfo, corpus []*Prog, ctrlnum int, counter int) {
	r := newRand(p.Target, rs)
	if ncalls < len(p.Calls) {
		ncalls = len(p.Calls)
	}
	...
	if counter == 0 {
		if len(procinfo.Targetlist) == 0 {
			ctx.initMutateinfo(procinfo)
			ctx.resizeMutateinfo(procinfo)
			Randomsplice(procinfo)
			//Randomsplice(procinfo.Waitlist))
		} else {
			res := ctx.updateMutateinfo(procinfo, ctrlnum)
			if res == false {
				panic("find var arg map set error!")
			}
			ctx.resizeMutateinfo(procinfo)
		}
		log.Logf(0, "targetlist len: %d, waitlist len: %d\n", procinfo.Targetlist, procinfo.Waitlist)
	} else {
		counter = 1
	}
	ok := false
	//for stop, ok := false, false; !stop; stop = ok && len(p.Calls) != 0 && r.oneOf(3) {
	// ctrlnum: 0, 1=> build map; 2 => mutate
	switch ctrlnum {
	case 0:
		ok = ctx.mutateArgbyinfo(*procinfo)
	case 1:
		ok = ctx.mutateArgbyinfo(*procinfo)
	case 2:
		ok = ctx.mutateArgbyinfo(*procinfo)
